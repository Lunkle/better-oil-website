              ...
              >
                ...
                {item.dropdown && hoveredNav === item.name && (
                  <div className="absolute top-full left-0 mt-0 pt-2 min-w-[240px]">
                    <div className="bg-brand-white border border-border shadow-lg rounded-sm overflow-hidden flex">
                        ...
                        {item.dropdown.map((parentItem) => {
                          ...
                          return (
                            <div
                              ...
                              className={`relative group ${hoveredParent?.parent === parentItem.parent ? 'bg-gray-50' : 'hover:bg-gray-50'}`}
                              onMouseEnter={() => setHoveredParent(parentItem)}
                            >
                              {parentItem.href ? (
                                <Link
                                  href={`${parentItem.href}?lang=${currentLang}`}
                                  className="block px-6 py-4 text-sm font-medium text-foreground hover:text-primary transition-colors"
                                >
                                  {parentItem.parent}
                                </Link>
                              ) : (
                                <div className="px-6 py-4 flex items-center justify-between text-sm font-medium text-foreground cursor-default">
                                  {parentItem.parent}
                      ...
                      {item.dropdown.some(p => p.children) && (
                        <div className="w-[300px] border-l border-border bg-gray-50/50 p-6">
                             ...
                             <div className="flex flex-col space-y-4">
                               {hoveredParent.children.map(child => (
                                 <Link
                                   ...
                                   className="group flex items-start text-sm text-foreground/80 hover:text-primary transition-colors"
                                 >
