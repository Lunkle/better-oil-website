...
export default function Item3({ data }: Item3Props) {
  return (
    <div className="relative w-full h-[600px] bg-white overflow-hidden rounded-xl border border-border p-8 flex gap-12">

      {/* Left Side: 3x2 Pillars */}
      <div className="w-2/3 grid grid-cols-2 grid-rows-3 gap-6">
        {data.leftPillars.map((pillar, idx) => (
          <div key={idx} className="flex flex-col h-full bg-slate-50 border border-slate-100 p-4 rounded-lg shadow-sm">
            ...
            <div className="mt-auto flex gap-2">
               {[1, 2].map(i => (
                 <div key={i} className="w-6 h-6 rounded bg-slate-200 flex items-center justify-center">
      ...
      <div className="w-1/3 bg-slate-800 rounded-xl p-8 text-white flex flex-col justify-center">
